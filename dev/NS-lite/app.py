...
class MarkdownBuffer:
    def __init__(self):
        ...
        self.buffer = ""
        self.markdowner = Markdown(extras=["fenced-code-blocks", "tables", "break-on-newline"])

    def add_token(self, token):
        # Check for section headers
        if "### " in token:
            # Flush existing content if any
            html = self.flush()
            self.current_section = token
            return html
    ...
    def _is_complete_block(self):
        # Check for common markdown block endings
        endings = ["\n\n", "\n### ", ". ", "? ", "! "]
        return any(ending in self.buffer for ending in endings)

    def flush(self):
        if not self.buffer.strip():
            return ""

        html = self.markdowner.convert(self.buffer)
        self.buffer = ""
        return f'<div class="markdown-content">{html}</div>'

def format_token_update(data):
    """Format token with proper markdown rendering"""
    if data['type'] == 'token':
        return data['content']
    return ""
...
<head>
    ...
    <style>
        ...
        /* Add a subtle highlight for section headers */
        .token-update:contains('### Reasoning Steps:'),
        .token-update:contains('### Final Answer:') {
            display: block;
            font-weight: bold;
            margin-top: 1em;
            padding: 0.5em 0;
...
<body style="font-family: sans-serif; margin: 40px; max-width: 1200px; margin: 0 auto; padding: 20px;">
    ...
    <script>
        ...
        function appendToken(token) {
            liveStream.insertAdjacentText('beforeend', token);
